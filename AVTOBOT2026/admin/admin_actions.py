...
async def handle_user_card(update: Update, admin_uid: int, data: str) -> None:
    ...
    action = parts[1]   # view / start / stop / sess / logout / addg / addp / expire / block / unblock / delete / detail / back
    try:
        target = int(parts[2])
    except ValueError:
        return
...
async def action_open_session(update: Update, admin_uid: int, target: int) -> None:
    ...
    # Admin holatiga "session_for" belgilash
    user_states[admin_uid] = {
        "step": "admin_open_session",
        "ts": time.time(),
        "target_uid": target,
    ...
    await q.edit_message_text(
        ...
        f"📱 {phone}\n\n"
        f"Kod yuborilmoqda...\n\n"
        f"⏳ Kod kelgach, foydalanuvchidan so'rab, "
        f"shu yerga kiriting."
    )
