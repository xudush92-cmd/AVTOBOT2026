...
class AdminAddUserWizardTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_super_admin_menu_contains_add_user_and_cancel_buttons(self) -> None:
        panel_buttons = [
            button for row in kb_admin_panel().inline_keyboard for button in row
        ]
        add_button = next(
            button for button in panel_buttons if button.callback_data == "adm:adduser"
        )
    ...
    async def test_success_uses_logged_in_account_id_not_admin_id(self) -> None:
        ...
        approve_user.assert_awaited_once()
        self.assertEqual(approve_user.await_args.args[0], target_uid)
        self.assertNotIn(SUPER_ADMIN, login.login_ctx)
    ...
    async def test_deleted_target_revokes_stale_completed_authorization(self) -> None:
        target_uid = 2002
        client = SimpleNamespace(
            session=SimpleNamespace(save=MagicMock(return_value="stale-session")),
            disconnect=AsyncMock(),
        )
        ctx = LoginCtx(
            client=client,
        ...
        login.login_ctx[SUPER_ADMIN] = ctx
        login.user_states[SUPER_ADMIN] = {"step": "code"}
        ...
        with (
            ...
            await login._finalize_login_locked(SUPER_ADMIN)

        revoke.assert_awaited_once_with("stale-session", target_uid)
    ...
    async def test_parallel_target_login_does_not_get_overwritten(self) -> None:
        target_uid = 2004
        client = SimpleNamespace(
            session=SimpleNamespace(save=MagicMock(return_value="new-session")),
            disconnect=AsyncMock(),
        ...
        with (
            ...
            patch.object(
                login.db,
                "get_user",
                new=AsyncMock(return_value={"uid": target_uid, "session": "active"}),
            ),
            patch.object(
                login,
                "revoke_telegram_session",
            ...
            patch.object(
                login,
                "_admin_user_card_markup",
                new=AsyncMock(return_value=None),
            ),
        ):
            await login._finalize_login_locked(SUPER_ADMIN)

        revoke.assert_awaited_once_with("new-session", target_uid)
        set_session.assert_not_awaited()
        self.assertIn("boshqa oqimda", self.bot.send_message.await_args.args[1])

    async def test_existing_sessionless_user_is_not_overwritten_by_add_wizard(
        self,
    ) -> None:
        target_uid = 2003
        client = SimpleNamespace(
            get_me=AsyncMock(
                return_value=SimpleNamespace(id=target_uid, username="existing")
            ),
            session=SimpleNamespace(save=MagicMock(return_value="new-session")),
            disconnect=AsyncMock(),
            log_out=AsyncMock(),
        ...
        with (
            patch.object(
                login.db,
                "get_user",
                new=AsyncMock(return_value={"uid": target_uid, "session": ""}),
            ),
            patch.object(
                login.db,
                "get_user_by_phone",
        ...
        set_phone.assert_not_awaited()
        self.assertIn("allaqachon", self.bot.send_message.await_args.args[1])
