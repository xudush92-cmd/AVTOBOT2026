...
class QrLoginLifecycleTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_cleanup_cancels_qr_waiter_and_disconnects(self) -> None:
        ...
        self.bot.delete_message.assert_awaited_once_with(chat_id=uid, message_id=77)
        self.assertNotIn(uid, login.login_ctx)
        self.assertNotIn(uid, login.user_states)

    async def test_cleanup_revokes_completed_temporary_authorization(self) -> None:
        uid = 1006
    ...
    async def test_qr_success_rejects_a_different_telegram_user(self) -> None:
        ...
        client = SimpleNamespace(
            get_me=AsyncMock(
                return_value=SimpleNamespace(id=9999, phone="998900000000")
            ),
            disconnect=AsyncMock(),
        )
        ctx = LoginCtx(
    ...
    async def test_qr_success_defers_phone_write_to_locked_finalize(self) -> None:
        uid = 1003
        client = SimpleNamespace(
            get_me=AsyncMock(
        ...
        self.assertEqual(ctx.phone, "+998900000000")

    async def test_code_login_binds_uid_and_authenticated_phone(self) -> None:
        uid = 1004
        client = SimpleNamespace(
            get_me=AsyncMock(
                return_value=SimpleNamespace(id=uid, phone="998901112244")
            ),
            log_out=AsyncMock(),
        )
        ctx = LoginCtx(
        ...
        with (
            ...
            self.assertTrue(await login._validate_signed_in_account(uid, ctx))
        set_phone.assert_awaited_once_with(uid, "+998901112244")
        self.assertEqual(ctx.phone, "+998901112244")
        client.log_out.assert_not_awaited()

    async def test_code_login_rejects_different_authenticated_uid(self) -> None:
        uid = 1005
        client = SimpleNamespace(
            get_me=AsyncMock(
                return_value=SimpleNamespace(id=999999, phone="998901112255")
            ),
            log_out=AsyncMock(),
        )
        ctx = LoginCtx(
        ...
        with patch.object(
            ...
            self.assertFalse(await login._validate_signed_in_account(uid, ctx))
        set_phone.assert_not_awaited()
        client.log_out.assert_awaited_once()

    async def test_parallel_session_completion_revokes_loser(self) -> None:
        uid = 1007
        client = SimpleNamespace(
            session=SimpleNamespace(save=lambda: "losing-session"),
            disconnect=AsyncMock(),
        )
        login.login_ctx[uid] = LoginCtx(
            client=client,
        ...
        revoke.assert_awaited_once_with("losing-session", uid)
