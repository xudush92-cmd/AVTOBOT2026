...
async def get_stats() -> dict:
    """Oddiy foydalanuvchilar statistikasi (super admin hisoblanmaydi)."""
    params = (SUPER_ADMIN,)

    async with _conn().execute(
        "SELECT COUNT(*) AS n FROM users WHERE uid != ?", params
    ) as cur:
    ...
    async with _conn().execute(
        ...
        "WHERE awaiting_approval = 1 AND uid != ?",
        params,
    ) as cur:
    ...
    async with _conn().execute(
        "SELECT COUNT(*) AS n FROM groups WHERE uid != ?", params
    ) as cur:
        groups = (await cur.fetchone())["n"]

    async with _conn().execute(
        "SELECT COUNT(*) AS n FROM posts WHERE uid != ?", params
    ) as cur:
...
async def get_all_users() -> list[dict]:
    """Super adminni qo'shmasdan oddiy foydalanuvchilarni qaytaradi."""
    async with _conn().execute(
